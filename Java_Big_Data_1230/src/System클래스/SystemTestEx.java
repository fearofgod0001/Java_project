...
public class SystemTestEx {
    public static void main(String[] args) throws InterruptedException{
        ...
        int cnt = 0;
        int val = (int)(Math.random() * 1000) +1; // 1~1000 사이 임의 값
        for(int i =0;i<data.length; i++){ // 1000개의 배열에 1부터 1000까지 값을 저장
            data[i] = i +1;
        }

        long time1 = System.currentTimeMillis(); //측정을 위한 시작 시간 가져오기
        for(int i = 0; i< data.length; i++){
            cnt++;
            sleep(1); //시간 측정에 대한 지연 요인을 만들기 위해 인위적으로
            if(val == data[i]){
        ...
        long time2 = System.currentTimeMillis(); //측정을 위한 시작 시간 가져오기
